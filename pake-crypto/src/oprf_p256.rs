...
fn point_from_bytes(bytes: &[u8]) -> Result<ProjectivePoint, PakeError> {
    let encoded =
        EncodedPoint::from_bytes(bytes).map_err(|_| PakeError::InvalidInput("invalid point encoding"))?;
...
fn reduce_48_to_scalar(bytes: &[u8; 48]) -> Scalar {
    ...
    let low_arr: [u8; 32] = bytes[16..].try_into().expect("bytes[16..] is exactly 32 bytes");
...
impl Oprf for P256Oprf {
    ...
    fn client_blind(
        ...
        Ok((
            P256OprfClientState { blind },
            point_to_bytes(&blinded),
        ))
    }
    ...
    fn derive_key(seed: &[u8], info: &[u8]) -> Result<Vec<u8>, PakeError> {
        ...
        for counter in 0u8..=255 {
            let sk = hash_to_scalar_with_dst(
                &[seed, &info_len, info, &[counter]],
                DERIVE_KEYPAIR_DST,
            )?;
            if !bool::from(sk.is_zero()) {
...
mod tests {
    ...
    fn invalid_key_length() {
        assert!(P256Oprf::server_evaluate(&[0u8; 16], &hex("03723a1e5c09b8b9c18d1dcbca29e8007e95f14f4732d9346d490ffc195110368d")).is_err());
    }
